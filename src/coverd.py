...
def CoVerD(graph, subg, target, idx):
    ...
    while sum(actions.values()) <= budget and not stop:
        ...
        out_neigh = sorted(
            [(node, G.degree(node)) for node in G[target] if node not in community],
            key=lambda x: x[1],
            reverse=True,
        )
        ...
        for neigh in orig_neigh:
            ...
            if sum(actions.values()) >= budget:
                break
            nscores = {
                node: community.degree(node) / G.degree(node)
        ...
        cand = sorted(
            [
                ((n1, n2), s1 - s2)
                for n1, s1 in nscores.items()
                for n2, s2 in nscores.items()
                if s1 == 1 and s2 < 1 and n1 != n2 and (n1, n2) in community.edges()
            ],
            key=lambda x: x[1],
            reverse=True,
        )
