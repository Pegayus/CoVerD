...
def get_comscore(graph,coms):
    ...
    # (3) 
    coms_score_overall = {k:(coms_score_nodes[k]+coms_score_edges[k])/2 for k in range(len(coms))}
...
def get_node(grpah,num,mode='rand',coms_score=None,coms=None):
    '''
    ...
    if mode == 'rand':
        ...
        probs = {n:coms_score[coms[n]] for n in graph.nodes()}  # all scores are already between 0 and 1
        sum_prob = sum(list(probs.values()))
...
if __name__ == '__main__':
    '''
    load each data
    get 100 targets for each random,degree,community strategies
    get 100 source nodes at random
    save as {graph:,targets_[some type]:,sources:}
    '''
    path = '../data/merged/'
    save2 = './data/'
    ...
    num_targets = 100
    num_sources = 100
    names = ['deezer.pkl','facebook_pages.pkl','github_web_ml.pkl','lastfm.pkl','twitch_en.pkl']
    ...
    for root,dirs,files in os.walk(path):
        ...
        for file in files:
            ...
            s = time()
            coms_score_edges, coms_score_nodes, coms_score_overall = get_comscore(graph,coms)
            print('community scoring done. elapsed: ', time()-s)
            res = dict(graph=graph,
                    targets_random = get_node(graph,num_targets,'rand'),
                    targets_degree = get_node(graph,num_targets, 'deg'),
                    targets_community = get_node(graph,num_targets, 'com',
                                                    coms=node_com,coms_score=coms_score_overall),
