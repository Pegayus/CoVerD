...
def get_args():
    ...
    # defender parameters
    parser.add_argument('--defend', type=str, default=['coverd'],
                choices=['coverd','roam','greedy','nothing','betweenness','pagerank','maxdeg'],
                help='Defender type. Default: coverd.', nargs='+')
            
    # attacker parameters
    parser.add_argument('--attack', type=str, default=['bfs'],
                choices=['bfs','dfs','rw'],
                help='Attacker type. Default: bfs.', nargs='+')
    
    # mode of running: defending and generating graph, or attacking a defended/existing graph
    parser.add_argument('--mode', type=str, default='attack',choices=['attack','defense'])
    
    # Handling data and results
    parser.add_argument('--name', type=str, default='lastfm', choices=DATA,
                help='The name of the dataset to be used by apath or dpath')
    parser.add_argument('--dpath', type=str, default='./data/raw', 
                help='Path to read the data for defender in pkl file from.')
    parser.add_argument('--apath', type=str, default='./data/defended',
                help='Path to directory to read the data for attacker in pkl file from.')
    parser.add_argument('--dsave2', type=str, default='./data/defended', 
                help='Path to directory to save the defender results.')
    parser.add_argument('--asave2', type=str, default='./results', 
...
if __name__ == "__main__":
    args = get_args()
    budgets = [round(0.001*i,3) for i in range(1,52,5)] # don't do from zero, that is "do nothing"
    ...
    if args.mode == 'defense': 
        ...
        for typ,tars in targets.items():
            ...
            for defender in args.defend:
                ...
                # name = defender_targetType.pkl
                name = f'{defender}_{typ}.pkl' 
                if defender != 'nothing':
                    res = {'sources':sources,'targets':{}} # targets: {targetnum:{graph:,com:}}
                    c = 0
                    for tar in tars:
                        print(f'    target node {c}:')
                        c += 1
                        subg = graph.subgraph([n for n,v in node_com.items() if v==node_com[tar]])
        ...
        '''
        path = os.path.join(args.apath,args.name) # walk in this
            ...
            for file in files:
                ...
                if 'nothing' not in file:
                    ...
                    for attack in args.attack:
                        ...
                        res = {'sources':sources,'targets':{}} # targets: {targetnum:(flag,list_queried)}}
                        ...
                        for tar,budggraph in tar_budggraph.items():
                            ...
                            for b_g in budggraph:
                                budg = b_g[0]
                                graph = b_g[1]
                                res['targets'][tar][budg] = ATTACKER[attack](graph,sources=sources,target=tar)
                        with open(os.path.join(save2,name),'wb') as f:
                            pickle.dump(res,f)
                else:
                    ...
                    for attack in args.attack:
                        ...
                        name = file.split('.pkl')[0] + '_' + attack + '.pkl'
                        for tar in tars:
                            res['targets'][tar] = ATTACKER[attack](graph,sources=sources,target=tar)
                        with open(os.path.join(save2,name),'wb') as f:
                            pickle.dump(res,f)
