...
# ROAM --- remove one, add many until target has degree 1 (extreme case)
def roam(graph,target,budget=1):
    ...
    sort the neighbors of the target by degree in ascending order. 
    Take the last node (highest degree) and remove from the list, and then remove the edge form target to it
    Take the fist "budget" nodes (lowest degrees) that are not in removed node neighbors
    Make an edge between removed node and the selected nodes.
    Return the graph.
    '''
...
def run_roam(graph,subg,target,budgets):
    ...
    for idx in budgets:
        ...
        e = time()-s
        result[idx] = {'graph':G.copy(),'community':community.copy(),'num_del_edges':actions['rem'],
...
def update_sorted_edges(sorted_edges,changed_edges,sorted_edge_names):
    '''
    Each changed edge has a drop of -1 in their score
    If the next edge after that changed edge in sorte_edges has a score that is now bigger than that, swap them
    '''
...
def run_greedy(graph,subg,target,budgets):
    ...
    sorted_edges = sorted([[i[0],i[1]] for i in edges_score.items()],
    ...
    with open(path,'wb') as f:
        pickle.dump(dict(graph=graph,
                        sorted_edges=sorted_edges,sorted_edge_names=sorted_edge_names,
                        used_budget = 0),f)
    count = 0
    for idx in budgets:
        count += 1
        ...
        with open(path,'rb') as f:
            rtrv = pickle.load(f) # retrieved materials from the last budget to speed up computation
        ...
        while not stop:
            ...
            changed_edges = [(node,neigh) if (node,neigh) in edges_score else (neigh,node) for 
                            node in candidate for neigh in G[node] if neigh not in candidate]
            sorted_edges, sorted_edge_names = update_sorted_edges(sorted_edges,changed_edges,sorted_edge_names)
        ...
        with open(path,'wb') as f:
            pickle.dump(dict(graph=G,
                            sorted_edges=sorted_edges,sorted_edge_names=sorted_edge_names,
                            used_budget = used_budget),f)

        e = time()-s
        result[idx] = {'graph':G.copy(),'community':None,'num_del_edges':used_budget,
                      'num_add_edges':0, 'dtime':e}
        print(f'\t budget {idx} done! Elapsed: {e}')
    return result
    ...
    for idx in budgets:
        ...
        for e in sorted_edge_names:
            ...
            if spent >= budget:
                break
        e = time()-s
        result[idx] = {'graph':G.copy(),'community':None,'num_del_edges':spent,
                      'num_add_edges':0, 'dtime':e}
        print(f'\t budget {idx} done! Elapsed: {e}')
    return result
...
def run_betweenness(graph,subg,target,budgets):
    ...
    if len(nodes) <= int(0.001*graph.number_of_edges()):
        ...
        escores = sorted(nx.edge_betweenness_centrality_subset(graph,
                        sources=nodes,targets=nodes).items(),
                        key=lambda x:x[1],reverse=True)
    else:
        escores = sorted(nx.edge_betweenness_centrality(graph,
                        k=int(0.001*graph.number_of_edges())).items(),
                        key=lambda x:x[1],reverse=True)
...
def run_pagerank(graph,subg,target,budgets):
    ...
    escores = sorted([(edge,nscores[edge[0]]+nscores[edge[1]]) for edge in graph.edges()],
                 key=lambda x:x[1],reverse=True)
    sorted_edge_names = [i[0] for i in escores]
    for idx in budgets:
        s = time()
